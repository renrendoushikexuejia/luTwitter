...
class luAutoFrame( QMainWindow, Ui_Form): 
        ...
        if os.path.exists( "./lu.ini"):
            try:
                ...
                if iniDict:
                    ...
                    self.cbCloseBrowser.setChecked( iniDict['cbCloseBrowser'])
            except:
    ...
    def mfStart( self):
        ...
        ISRUN = 1
        # 创建一个新线程来运行自动化操作
        if self.leTwitterName.text().split('+')[0] == '':
            QMessageBox.about( self, "提示", "请输入要关注的账号")
            return
    ...
    def mfRun(self):
        ...
        for i in browserList:
            ...
            bd.get("https://twitter.com/home")
            self.signalThread.emit( 'Display', browserName + " 打开窗口")

            twitterNameList = self.leTwitterName.text().split('+')
            self.signalThread.emit( 'Display', browserName + " 开始关注Twitter账号: " + str(twitterNameList))
            for tName in twitterNameList:     
                MyFake.useful_fake_mouse( 'clickOffset', 550, 25, 250, 12)      #点一下浏览器的窗口。获得操作焦点
                MyFake.useful_fake_mouse( 'moveTo', 300, 500, 200, 300)       #把鼠标放在窗口内,否则下一步scroll不成功
                MyFake.fake_mouse('scroll', 2)      #测试环境使用
                # MyFake.fake_mouse('scroll', 4)   #生产环境使用
                MyFake.useful_fake_mouse('clickOffset', 860, 136, 100, 20)     #点击搜索栏
                MyFake.fake_time( 's', 1, 2)
                MyFake.useful_fake_typeWrite( tName)        #输入twitter账户名，要@开头，否则会搜出来很多
                MyFake.fake_time( 's', 3, 6)
                MyFake.useful_fake_mouse( 'clickOffset', 860, 260, 140, 30)      #点击搜到的第一个名字
                MyFake.fake_time( 's', 10, 15)      #等待页面加载

                #检测follow按钮的颜色
                #follow按钮的颜色刚加载页面是39,44,48近似值, 空白的地方点击一下变成15,20,25
                #检测点(600,400) (650,410) (615,380) (660,385)   颜色15,20,25
                #点击一下之后,变为白色following.
                #检测点(600,400) (650,410) (615,380) (660,385)  颜色255,255,255
                self.signalThread.emit( 'Display', browserName + " 开始检测follow按钮颜色 " + tName)
                MyFake.useful_fake_mouse( 'clickOffset', 620, 370, 60, 4)      #点击空白,使follow颜色从39,44,48近似值变为15,20,25
                MyFake.fake_time( 's', 1, 3)
                currentRGBDict = getColor(600, 400)
                self.signalThread.emit( 'Display', browserName + " 检测(600,400)颜色为 " + str(currentRGBDict))
                if currentRGBDict['r'] == 255 and currentRGBDict['g'] == 255 and currentRGBDict['b'] == 255:
                    self.signalThread.emit( 'Display', browserName + " 已关注 " + tName)
                elif currentRGBDict['r'] == 15 and currentRGBDict['g'] == 20 and currentRGBDict['b'] == 25:
                    MyFake.fake_time( 's', 1, 3)
                    MyFake.useful_fake_mouse( 'clickOffset', 621, 394, 34, 14)      #点击follow按钮
                    self.signalThread.emit( 'Display', browserName + " 已点击Follow " + tName)
                else:
                    self.signalThread.emit( 'Display', browserName + " 检测follow按钮颜色出错 " + tName)
                
                MyFake.fake_mouse( 'scroll', 4)
                MyFake.useful_fake_mouse( 'clickOffset', 116, 138, 11, 11)
                MyFake.fake_time( 's', 1, 2)
            ...
            MyFake.fake_time( 's', 1, 2)
            if self.cbCloseBrowser.isChecked() == False:        #cbCloseBrowser选中时执行关闭操作,否则开始下次循环
                continue
            #######################################################
...
if __name__ == "__main__":
    ...
    tempDict = { 'URL':myWin.leURL.text(), 'sbX':myWin.sbX.value(), 'sbY':myWin.sbY.value(),
                    ...
                    'cbCloseBrowser': myWin.cbCloseBrowser.isChecked()}
